...
export const generateGameQuestions = async (
  ...
  count: number = 3
    ...
    let prompt = `Generate a strict ${count}-question exam paper on "${topic}" based on the sources.\n`;

    if (mode === 'MCQ_ARENA') {
        prompt += `Include ONLY Multiple Choice Questions (MCQ).\n`;
    } else if (mode === 'EXPLAIN_TO_WIN') {
        prompt += `Include ONLY Open Ended questions requiring explanation.\n`;
    } else {
        prompt += `Include a mix of questions.\n`;
    }

    prompt += `Assign marks (e.g., [1 mark], [4 marks]).\nReturn JSON.`;
