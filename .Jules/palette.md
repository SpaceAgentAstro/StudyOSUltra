...
**Learning:** Block-level interactive `div` wrappers reduce keyboard and screen reader accessibility.
**Action:** When creating interactive cards or list items, use a `<button>` tag, neutralized with `w-full text-left`, and avoid nesting other interactive elements (like an inner `<button>`).
