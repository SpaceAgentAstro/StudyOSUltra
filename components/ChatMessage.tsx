...
interface ChatMessageProps {
  message: Message;
  files: FileDocument[];
  onExplain: (code: string, agent: AgentRole) => void;
...
const ChatMessage: React.FC<ChatMessageProps> = React.memo(({
  ...
  /*
    ⚡ Bolt Optimization: Memoize parsed content
    💡 What: Extracting and memoizing the regex parsing and mapping logic at the top level.
    🎯 Why: Prevents expensive string splits, regex evaluation, and DOM node recreation during rapid AI streaming or when unrelated props (like isThinkingMode) change, while abiding by React Rules of Hooks.
    📊 Impact: Reduces CPU overhead per chunk during AI streaming and eliminates redundant parsing.
  */
  const parsedContent = useMemo(() => {
    if (!msg.text) return null;
    return msg.text.split(/(```[\s\S]*?```)/g).map((blockPart, blockIdx) => {
        // Code Block Handling
      ...
      <div className="max-w-[85%] space-y-2">
        <div className={`p-4 rounded-2xl shadow-sm relative
            ...
            <div className="flex flex-col gap-2 mt-2">
                ...
                {isThinkingMode && <span className="text-xs text-indigo-400 font-medium animate-pulse">Thinking deeply...</span>}
                {isSearchMode && <span className="text-xs text-blue-400 font-medium animate-pulse">Searching the web...</span>}
            </div>
            ) : (
