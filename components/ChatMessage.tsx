...
interface ChatMessageProps {
  ...
  showThinkingInfo: boolean;
  showSearchInfo: boolean;
}

const ChatMessage: React.FC<ChatMessageProps> = memo(({ msg, files, onExplainCode, showThinkingInfo, showSearchInfo }) => {
    ...
    <div className={`flex gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
        ...
        <div className={`p-4 rounded-2xl shadow-sm relative
            ...
            <div className="flex flex-col gap-2 mt-2">
                ...
                {showThinkingInfo && <span className="text-xs text-indigo-400 font-medium animate-pulse">Thinking deeply...</span>}
                {showSearchInfo && <span className="text-xs text-blue-400 font-medium animate-pulse">Searching the web...</span>}
            </div>
            ) : (
                        ...
                        return (
                            <div key={blockIdx} className="my-3 rounded-lg border border-slate-200 overflow-hidden bg-slate-50 group">
                                <div className="flex items-center justify-between px-3 py-2 bg-slate-100 border-b border-slate-200">
                                    ...
                                    <button
                                        onClick={() => onExplainCode(codeContent)}
