...
const FileUploader: React.FC<FileUploaderProps> = ({ files, setFiles }) => {
  ...
  // What: Memoized file filtering and hoisted searchQuery lowercasing
  // Why: Prevents O(N) re-evaluation of the file list on every render (e.g., when file progress updates) and avoids redundant string conversion per file
  // Impact: Reduces CPU cycles during render, especially critical when many files are present and uploading simultaneously
