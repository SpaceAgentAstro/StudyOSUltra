...
const FileUploader: React.FC<FileUploaderProps> = ({ files, setFiles }) => {
  ...
  const simulateIngestionPipeline = (id: string, file: File) => {
    ...
    setTimeout(() => {
        ...
        updateStep(file.name.endsWith('.pdf') ? 'Running OCR & Text Extraction...' : 'Reading Content...');
        ...
        reader.onload = (e) => {
            ...
            setTimeout(() => {
                // Step 3: Chunking
                updateStep('Page-aware Chunking & Metadata Extraction...');
                
                setTimeout(() => {
                    // Step 4: Vector Embeddings
                    updateStep('Generating Vector Embeddings...');
