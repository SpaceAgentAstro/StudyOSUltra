...
const KnowledgeUniverse: React.FC<KnowledgeUniverseProps> = ({ files }) => {
  ...
  // ⚡ Bolt Optimization: Memoize SVG edge calculations and use a Map for O(1) lookups.
  // Impact: Reduces edge calculation time complexity from O(N^2) to O(N) by eliminating nested .find() calls.
  // This prevents costly layout recalculations during rapid re-renders (e.g., hover states or subsequent API updates).
