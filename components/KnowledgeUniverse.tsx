...
const KnowledgeUniverse: React.FC<KnowledgeUniverseProps> = ({ files }) => {
  ...
  // ⚡ Bolt Optimization: Replace O(N^2) nested find with O(N) Map lookup + Memoization
  // Expected Impact: Reduces line generation complexity from O(N^2) to O(N) and prevents
  // recalculation on every render tick (e.g., when hover state/tooltips change).
