  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 h-[500px]">
        <button
          className="bg-indigo-900 rounded-3xl p-8 text-white flex flex-col justify-between relative overflow-hidden group cursor-pointer text-left focus:outline-none focus:ring-4 focus:ring-indigo-500/50"
          onClick={() => setCurrentView(AppView.KNOWLEDGE_UNIVERSE)}
          aria-label="Enter Knowledge Universe"
        >
        ...
        <div className="bg-white rounded-3xl border border-slate-200 p-8 flex flex-col">
          ...
          <div className="space-y-3">
            <button
              className="w-full text-left p-4 rounded-xl bg-slate-50 hover:bg-indigo-50 border border-slate-100 transition-colors cursor-pointer flex items-center justify-between group focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
              onClick={() => setCurrentView(AppView.CHAT)}
              aria-label="Talk to The Coach. Reduce exam anxiety"
            >
              <div>
              ...
              <div className="w-8 h-8 rounded-full bg-white flex items-center justify-center text-slate-400 group-hover:text-indigo-600 shadow-sm" aria-hidden="true">→</div>
            </button>
            <button
              className="w-full text-left p-4 rounded-xl bg-slate-50 hover:bg-indigo-50 border border-slate-100 transition-colors cursor-pointer flex items-center justify-between group focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
              onClick={() => setCurrentView(AppView.META_LEARNING)}
              aria-label="Meta Analysis. Check for cognitive biases"
            >
              <div>
