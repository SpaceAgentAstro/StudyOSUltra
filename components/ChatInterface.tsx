...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ files, initialMessages = [], onMessagesChange }) => {
  ...
  // Keep a stable reference to handleSend for MessageContent callbacks
  const handleSendRef = useRef(handleSend);
  useEffect(() => {
    handleSendRef.current = handleSend;
  });

  const onExplain = useCallback((code: string) => {
            ...
            <div key={msg.id} className={`flex gap-4 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                ...
                <div className={`p-4 rounded-2xl shadow-sm relative
                    ...
                    <div className="flex flex-col gap-2 mt-2">
                        ...
                        <MessageContent text={msg.text} files={files} onExplain={onExplain} />
                    )}
                </div>
